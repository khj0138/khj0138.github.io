...
## View 변환
* World Space 내에 존재하는 Game Object들을 Level 내 메인 Camera의 View Space 좌표계로 변환<br>
* 카메라가 바라보는 정면은 Z축과 일치해야 함, 카메라의 World Space 위치가 원점이 되어야 함<br>
-> 카메라가 바라보는 정면을 Z축과 일치시키기 위해 카메라의 World 회전 행렬을 항등 행렬로 변환시켜야 함<br>
-> 또한, 카메라의 World 좌표를 원점으로 변환시켜야 함<br>
-> 카메라의 회전 행렬을 항등 행렬로 변환했을 때, 카메라의 정면은 Z축과 일치함<br>
->회전 행렬의 X축, Y축, Z축 회전 행렬은 서로 직교하므로, 회전 행렬의 역행렬은 회전 행렬의 전치 행렬과 같음<br>
-> 또한, 카메라의 World 좌표의 음수값만큼 이동시키면 카메라의 World 좌표는 원점으로 이동함<br>
-> 결론적으로 모든 Game Object에 적용되는 View 행렬은 다음 이미지와 같음<br>
